$(document).ready(function() {
  ...
  chrome.browserAction.setBadgeText({ text: "TR" });
  chrome.browserAction.setBadgeBackgroundColor({ color: "#FC636B" });
  chrome.cookies.get({
    url: CONSTANTS.getBaseApiUrl(),
    name: CONSTANTS.ASANA_LOGIN_COOKIE_NAME
  }, function (cookie) {
  ...
  chrome.cookies.onChanged.addListener(function (changeInfo) {
    if (CONSTANTS.isAsanaDomain(changeInfo.cookie.domain) && CONSTANTS.isAsanaLoginCookie(changeInfo.cookie.name)) {
      CONSTANTS.setLoggedIn(!changeInfo.removed);
    }
  });
          ...
          var promise = AsanaGateway.search(options).then(function (response) {
            if (response && response.length) {
              response.forEach(function (task) {
                ...
                suggestion.content = CONSTANTS.getAsanaDomain() + "0/" + task.gid + "/" + task.gid;
