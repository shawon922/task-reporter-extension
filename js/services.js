...
AsanaGateway.getWorkspaceUsers = function (options) {
    ...
    return AsanaGateway.api(options).then(function (response) {
        CONSTANTS.setDefaultPicture(response);
        return response;
    });
};
...
AsanaGateway.getWorkspaceProjects = function (options) {
    ...
    return AsanaGateway.api(options).then(function (response) {
        ...
        var hideArchivedProjects = CONSTANTS.getHideArchivedProjects();
...
AsanaGateway.getTask = function (options) {
    ...
    return AsanaGateway.api(options).then(function (task) {
        CONSTANTS.setDefaultPictureUser(task.assignee);
        CONSTANTS.setDefaultPicture(task.followers);
        return task;
    });
};
...
AsanaGateway.getTaskStories = function (options) {
    ...
    return AsanaGateway.api(options).then(function (stories) {
        stories.forEach(function (story) {
            CONSTANTS.setDefaultPictureUser(story.created_by);
        });
        return stories;
    });
};
...
AsanaGateway.api = function (options) {
    ...
    var url = CONSTANTS.getBaseApiUrl() + options.path + "?" + queryParams;
...
ChromeExtensionService.openLink = function (url) {
    chrome.tabs.create({url: url}, function () {
        window.close();
    });
};

ChromeExtensionService.openLinkInTab = function (url, tab) {
    chrome.tabs.update(tab.id, {url: url});
};

ChromeExtensionService.getCurrentTab = function (callback) {
